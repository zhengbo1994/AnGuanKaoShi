...
namespace BLL
{
    public class RP_WorkFlowCtrl : IRP_WorkFlowCtrl
    {
        ...
        public IQueryable<Biz_Certificate> GetCertificateWorkFlowStatusQueryable(string workflowStatus)
        {
            IQueryable<Biz_Certificate> certificateQueryable = uow.Biz_Certificate.GetAll();
            if (!workflowStatus.IsNull())
            {
                if (workflowStatus == RP_WorkFlowCtrl.WorkFlowStatusTag.RegisterDraft)
                {
                    IQueryable<Biz_CertificateDelayApplyRecord> certificateDelayApplyRecordQueryable = uow.Biz_CertificateDelayApplyRecord.GetAll().Where(p => p.SubmitStatus == false);
                ...
                if (workflowStatus == RP_WorkFlowCtrl.WorkFlowStatusTag.RegisterSummit)
                {
                    IQueryable<Biz_CertificateDelayApplyRecord> certificateDelayApplyRecordQueryable = uow.Biz_CertificateDelayApplyRecord.GetAll().Where(p => p.SubmitStatus == true && p.OperationStatus == false);
                    certificateQueryable = certificateQueryable.Join(certificateDelayApplyRecordQueryable, a => a.Id, b => b.CertificateId, (a, b) => new { a, b }).Select(o => o.a);
                }
                if (workflowStatus == RP_WorkFlowCtrl.WorkFlowStatusTag.DataCheckUnpassed)
                {
                    IQueryable<Biz_RP_EmployeeDataCheckedRecord> rpEmployeeDataCheckedRecordQueryable = uow.Biz_RP_EmployeeDataCheckedRecord.GetAll().Where(p => p.PassStatus == false && p.OperationStatus == false);
                    certificateQueryable = certificateQueryable.Join(rpEmployeeDataCheckedRecordQueryable, a => a.Id, b => b.CertificateId, (a, b) => new { a, b }).Select(o => o.a);
                }
                if (workflowStatus == RP_WorkFlowCtrl.WorkFlowStatusTag.DataCheckPassed)
                {
                    IQueryable<Biz_RP_EmployeeDataCheckedRecord> rpEmployeeDataCheckedRecordQueryable = uow.Biz_RP_EmployeeDataCheckedRecord.GetAll().Where(p => p.PassStatus == true && p.OperationStatus == false);
                    certificateQueryable = certificateQueryable.Join(rpEmployeeDataCheckedRecordQueryable, a => a.Id, b => b.CertificateId, (a, b) => new { a, b }).Select(o => o.a);
                }
                if (workflowStatus == RP_WorkFlowCtrl.WorkFlowStatusTag.CheckUnpassed)
                {
                    IQueryable<Biz_RP_EmployeeCheckedRecord> rpEmployeeCheckedRecordQueryable = uow.Biz_RP_EmployeeCheckedRecord.GetAll().Where(p => p.PassStatus == false && p.OperationStatus == false);
                    certificateQueryable = certificateQueryable.Join(rpEmployeeCheckedRecordQueryable, a => a.Id, b => b.CertificateId, (a, b) => new { a, b }).Select(o => o.a);
                }

                if (workflowStatus == RP_WorkFlowCtrl.WorkFlowStatusTag.CheckPassed)
                {
                    IQueryable<Biz_RP_EmployeeCheckedRecord> rpEmployeeCheckedRecordQueryable = uow.Biz_RP_EmployeeCheckedRecord.GetAll().Where(p => p.PassStatus == true && p.OperationStatus == false);
                    certificateQueryable = certificateQueryable.Join(rpEmployeeCheckedRecordQueryable, a => a.Id, b => b.CertificateId, (a, b) => new { a, b }).Select(o => o.a);
                }
            }
            else
        ...
        //public List<RP_WorkFlowCtrl.WorkFlowStatus> GetCurrentWorkFlowStatusByCertificateIdList(List<int> certificateIdList)
        //{
        //    List<WorkFlowStatus> WorkFlowStatusListResult = new List<WorkFlowStatus>();
        //    //审核
        //    {
        //        List<Biz_CertificateDelayApplyRecord> certificateDelayApplyRecord = uow.Biz_CertificateDelayApplyRecord.GetAll().Where(p => certificateIdList.Contains(p.CertificateId)).ToList();
        //        if (certificateDelayApplyRecord.Count() > 0)
        //        {
        //            List<WorkFlowStatus> WorkFlowStatusList = rpEmployeeCheckedRecordList.Select(p => new WorkFlowStatus()
        //            {
        //                RPEmployeeId = p.EmployeeId,
        ...
        {
            public static string RegisterDraft { get { return "延期申请未提交"; } }
            public static string RegisterSummit { get { return "延期申请已提交"; } }
