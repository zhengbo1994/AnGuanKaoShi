        ...
        public JsonResult UserLogin(string loginname, string password, string validateCode)
        {
            ...
            try
            {
                ...
                if (loginFlag)
                {
                    ...
                    if (account.RoleList.Where(p => p.RoleType == RoleCtrl.RoleType.TrainingInstitution).Count() > 0)
                    {
                        ITrainingInstitutionCtrl trainingInstitutionCtrl = new  TrainingInstitutionCtrl(account);
                       bool result= trainingInstitutionCtrl.VerifyTrainingInstitutionAccredit(account.UserId);
                       if (!result)
                       {
                           throw new Exception("未授权的培训机构,不能登录");
                       }
                    }
